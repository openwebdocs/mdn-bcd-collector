...
const remapPropertyValues = (input, types) => {
  ...
  for (const val of resolveValuesFromTypes(input, types)) {
    if (
      ["inherit", "initial", "revert", "revert-layer", "unset", ...ignoredColorNames].includes(val)
    ) {
